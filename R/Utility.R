...
gradUtilityLambda <- function(omega, omega_0, k, arrivals, epsilon, lambda_0, beta){
	...
	term_1 <- 0
	for(i in 1:length(k)){
		term_1 <- term_1 - (beta /(omega_0 + (omega / k[i]))) * (exp( -(omega_0 + (omega / k[i]))* 
  			(arrivals[n] - arrivals[i])) - 1)
  	}
  	first_term <- 2*(n - term_1)
  	second_term <- -(lambda_0 / epsilon) * (exp(-epsilon * arrivals[n]) - exp(-epsilon * arrivals[1]))
...
gradUtilityBeta <- function(omega, omega_0, k, arrivals, epsilon, lambda_0, beta){
	...
	term_2 <- 0
	for(i in 1:length(k)){
		term_2 <- term_2 - (1 /(omega_0 + (omega / k[i]))) * (exp( -(omega_0 + (omega / k[i]))* 
  			(arrivals[n] - arrivals[i])) - 1)
  	}
